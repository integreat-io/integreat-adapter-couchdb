...
test('should set rev for one item', async (t) => {
  ...
  const source = createSource({id: 'entries'})
  sinon.stub(source, 'retrieve').resolves({
    status: 'ok',
  ...
  t.is(source.retrieve.callCount, 1)
  t.deepEqual(source.retrieve.args[0][0], expectedReq)
})
...
test('should handle error on rev endpoint', async (t) => {
  ...
  const source = createSource({id: 'entries'})
  sinon.stub(source, 'retrieve').resolves({status: 'error', error: 'Server failed'})
